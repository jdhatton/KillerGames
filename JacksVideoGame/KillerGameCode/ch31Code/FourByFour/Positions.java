...
public class Positions 
{
   ...
   /* Load point array with the offsets (coordinates) for each of 
      the NUM_SPOTS positions. */
   {
   ...
   /* Create the small white spheres that mark unoccupied positions.
   ...
   */
   {
      ...
      for (int i=0; i<NUM_SPOTS; i++) {
         ...
         shape.setUserData( new Integer(i) ); // add board position ID to each shape
  ...
  {
    // System.out.println("Player " + player + " clicked on position " + pos);
    ...
    if (player == PLAYER1) {
       player1Mask.set(pos);
       player1Switch.setChildMask(player1Mask);   // red sphere for p1
    }
    else if (player == PLAYER2) {   
       player2Mask.set(pos);
       player2Switch.setChildMask(player2Mask);   // blue cube for p2
    }
    else 
